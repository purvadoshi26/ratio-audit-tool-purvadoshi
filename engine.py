...
# ── FILE LOADER ───────────────────────────────────────────────
_HEADER_KEYWORDS = ["account name", "account", "particulars", "description", "head", "ledger",
                     "cy amount", "py amount", "current year", "prior year", "category", "type"]

def _detect_header_row(raw, max_scan=15):
    """Scan the first few rows for the one that looks like the real column header
    (handles files that have a title / instructions row above the header)."""
    best_row, best_score = 0, -1
    for i in range(min(max_scan, len(raw))):
        vals = [str(v).strip().lower() for v in raw.iloc[i].tolist() if pd.notna(v)]
        score = sum(1 for v in vals for kw in _HEADER_KEYWORDS if v == kw or (len(kw) > 4 and kw in v))
        if score > best_score:
...
def load_tb(filepath: str):
    ...
    header_vals = [str(v).strip() if pd.notna(v) else f"Unnamed {i}" for i, v in enumerate(raw.iloc[hdr_row].tolist())]
    ...
    if df.empty:
        raise ValueError("No data rows found. Make sure CY Amount / PY Amount are filled in "
                          "(not blank) for at least one account, then re-upload.")
