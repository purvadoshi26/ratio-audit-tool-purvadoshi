...
TEMPLATE_PATH = os.path.join(APP_DIR, "TB_Input_Template.xlsx")
...
with tab_main:
    ...
    with col_info:
        ...
        if os.path.exists(TEMPLATE_PATH):
            with open(TEMPLATE_PATH, "rb") as tf:
                st.download_button(
                    "⬇️ Download blank TB template",
                    data=tf,
                    file_name="TB_Input_Template.xlsx",
    ...
    if uploaded is not None:
        ...
        with r1:
            if not ratios:
                ...
                if available_ratios.empty:
                    ...
                    st.dataframe(
                        disp.style.map(style_flag, subset=["Flag"] if "Flag" in disp.columns else []),
        ...
        with r2:
            ...
            st.dataframe(
                disp_all.style.map(style_flag, subset=["Flag"]),
        ...
        with r3:
            ...
            if flagged.empty:
                ...
                disp_f["Change (%)"] = disp_f["Change (%)"].apply(
                    lambda x: "New" if x == 999.0 else (f"{x:+.1f}%" if isinstance(x, float) else x))
                st.dataframe(
                    disp_f.style.map(style_flag, subset=["Flag"]),
        ...
        with r4:
            ...
            if not gc_df.empty:
                st.dataframe(
                    gc_df.style.map(style_flag, subset=["Status"] if "Status" in gc_df.columns else []),
...
with tab_bf:
    ...
    if bf_file is not None:
        ...
        if not bf_result.get("sufficient"):
            ...
            st.dataframe(
                digit_df.style.map(style_flag_bf, subset=["Deviation (pp)"] if "Deviation (pp)" in digit_df.columns else []),
