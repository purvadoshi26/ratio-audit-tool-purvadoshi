...
with tab_main:
    ...
    if uploaded is not None:
        from engine import load_tb, run_analysis
        ...
        if len(unclass):
            with st.expander(f"⚠️ {len(unclass)} accounts could not be auto-classified"):
                st.dataframe(unclass[["Account Name", "CY Amount", "PY Amount"]], use_container_width=True)
                st.caption("Add a Category column to fix these. See Format Guide for valid values.")
